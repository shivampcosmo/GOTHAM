...
#CHECKPOINT     = '/work/hdd/bdne/yzhang116/checkpoints_quijote/1848113/checkpoint_w_cross_entropy_epoch_1_step_54000_embed_384_batch_800_lrmax_0.0001_lrmin_1e-05_layer_8_head_12_layervit_4_headvit_8_patch_4_dropout_0.0.pt'
...
SIM_ID         = 1414   # None → random cosmology; int → specific sim (e.g. 0)
...
N_BATCH        = 1200    # sub-boxes per forward pass; reduce if OOM
SEED         = 66
N_HALOS_SELECT = int(sys.argv[1]) if len(sys.argv) > 1 else 5
# ============================================================
...
# ---- Pick simulation ----
sim_numbers = sorted([int(f) for f in os.listdir(DMO_DIR)])
all_dm_files = [DMO_DIR + f'/{sim}/dmo_fields_subvols_grid_8_LH_{sim}.npy' for sim in sim_numbers]
...
n_halos = (end_token_index-7) // 8
L_seq    = gal_all.shape[1] - 1   # length of X (= gal[:, :-1])
D_sp     = DM_all.shape[1]        # spatial side length of DM field

select_index = np.where((n_halos == N_HALOS_SELECT) & ((end_token_index - 7) % 8 == 0))[0]
...
def make_cross_hook():
    def hook(module, args, kwargs, output):
        ...
        cross_attn_store.append(att)               # (B, n_head, Td, Te) float32
    return hook
...
for batch_start in range(0, N_subbox, N_BATCH):
    ...
    # end_token must land exactly on a halo-block boundary (7 header tokens,
    # then 8 tokens per halo); otherwise the sentence is corrupt and must be
    # dropped entirely rather than counted as valid.
    aligned       = ((raw_end - 7) % 8) == 0                             # (B,)
    if (has_end & ~aligned).any():
        n_bad = (has_end & ~aligned).sum().item()
        print(f'  WARNING: {n_bad} sub-box(es) have end_token at a non-boundary '
              f'position ((raw_end-7)%8 != 0); excluding from stats')
    ...
    valid_mask    = valid_mask & (~(has_end & ~aligned))[:, None]        # drop misaligned sentences entirely

    # Density sort for all B samples at once (float32 for mean)
    dm_ch = DM[:, DENSITY_CH].float()   # (B, D, H, W)
    ...
    # average over 2*2*2 kenel, stride 2, no padding
    ...
    # Alternative: average over 5*5*5 kernel, stride 1, no padding → (B, 4, 4, 4) → flatten to (B, 64)
    patch_density = F.avg_pool3d(
        dm_ch.unsqueeze(1),   # (B,1,8,8,8)
        kernel_size=5,
        stride=1,
        padding=0
    ).squeeze(1).view(B, -1)   # (B,4,4,4)
    '''

    sort_idx_batch = torch.argsort(patch_density,descending=True,dim=1)
    ...
    valid_f         = (valid_mask.float()[None, :, None, :, None])     # (1, B, 1, L_seq, 1)
    ca_masked       = ca_reordered * valid_f                          # (n_layers, B, n_head, Td, Te)
    cross_sum      += ca_masked.sum(dim=1).cpu().numpy()                    # (n_layers, n_head, Td, Te)
...
def plot_cross_avg_grid(attn_np, title, save_path):
    ...
    for hi in range(n_heads):
        ...
        im = ax.imshow(
            ...
            origin='upper', interpolation='nearest', vmin=0, vmax=0.16
        )
...
for li in range(n_layers):
    ...
    im = ax.imshow(attn_head_avg[li], cmap='viridis', aspect='auto',
                   origin='upper', interpolation='nearest', vmin=0, vmax=0.1)
...
fig.suptitle(
    f'Avg Cross-Attention (all layers & heads) — Sim {chosen_sim}  ({N_subbox} sub-boxes {N_HALOS_SELECT} halos in sub-box)\n'
    f'x: [cosmo ×{N_COSMO_TOKENS} | {N_patches} patches, density ↓]  |  y: decoder token position',
    fontsize=10,
)
...
plt.tight_layout()
save_path = os.path.join(OUT_DIR, f'cross_avg_all_cosmo_{chosen_sim}_{N_HALOS_SELECT}halos.png')
plt.savefig(save_path, dpi=300, bbox_inches='tight')
plt.close(fig)
print(f'Saved: {save_path}')
...
np.save(os.path.join(OUT_NPY_DIR, f'cross_avg_layer_head_cosmo_{chosen_sim}_{N_HALOS_SELECT}halos.npy'), attn_all_avg)
np.save(os.path.join(OUT_NPY_DIR, f'cross_avg_head_mean_per_layer_{chosen_sim}_{N_HALOS_SELECT}halos.npy'), attn_head_avg)

# Per-layer, per-head attention (no averaging over layers/heads)
attn_per_layer_head = cross_avg[:, :, :valid_rows, :]   # (n_layers, n_heads, valid_rows, Te)
np.save(os.path.join(OUT_NPY_DIR, f'cross_attn_per_layer_head_cosmo_{chosen_sim}_{N_HALOS_SELECT}halos.npy'), attn_per_layer_head)
print(f'\nAll figures saved to: {OUT_DIR}')
