...
SIM_ID         = int(sys.argv[1]) if len(sys.argv) > 1 else 66   # None → random cosmology; int → specific sim (e.g. 0)
DENSITY_CH     = 10     # 0-indexed DM field channel for density ranking
N_COSMO_TOKENS = 7      # ViT encoder prepends 5 fixed cosmology tokens before patches
N_BATCH        = 600    # sub-boxes per forward pass; reduce if OOM
SEED         = 66
N_HALOS_SELECT = 1
# ============================================================
...
'''
select_index = np.where(n_halos == N_HALOS_SELECT)[0]
DM_all = DM_all[select_index]
gal_all = gal_all[select_index]
N_subbox = DM_all.shape[0]
'''
...
def make_self_hook():
    def hook(module, args, kwargs, output):
        xd = args[0]
        maskd = kwargs.get('maskd')
        B, Td, C = xd.shape
        n_head    = module.n_head
        head_dim  = C // n_head
        dev = xd.device.type
        attn_causal_mask = torch.tril(torch.ones(Td, Td)).view(1, 1, Td, Td).to(xd.device)
        ...
        with torch.no_grad(), torch.amp.autocast(device_type=dev, enabled=False):
            ...
            k = module.k_attn(xd.float()).view(B, Td, n_head, head_dim).transpose(1, 2)
            att = (q @ k.transpose(-2, -1)) / math.sqrt(head_dim)
            mask_causal = attn_causal_mask[:,:,:Td,:Td].expand(B, n_head, Td, Td)
            if maskd is None:
                attn_mask = mask_causal
            else:
                attn_mask = mask_causal + maskd.view(B, 1, 1, Td).expand(B, n_head, Td, Td)
            att = att + attn_mask
...
for block in model.transformer.h:
    hooks.append(block.selfattn.register_forward_hook(make_self_hook(), with_kwargs=True))
...
for batch_start in range(0, N_subbox, N_BATCH):
    ...
    ca_masked       = ca_all * valid_f                          # (n_layers, B, n_head, Td, Te)
...
def _ax_setup(ax, im, title_str):
    ...
    ax.axhline(y=N_COSMO_TOKENS - 0.5, color='red', linestyle='-', linewidth=1.2, alpha=0.9)
    ax.set_title(title_str, fontsize=9)
    ax.set_xlabel('Decoder token position', fontsize=7)
    ax.set_ylabel('Decoder token position', fontsize=7)
    ...
    ax.set_xticklabels(xpos_labels, fontsize=5, color='cyan')
...
plt.tight_layout()
save_path = os.path.join(OUT_DIR, 'self_avg_head_mean_per_layer.png')
plt.savefig(save_path, dpi=300, bbox_inches='tight')
plt.close(fig)
print(f'Saved: {save_path}')
...
attn_all_avg = cross_avg[:, :, :valid_rows, :valid_rows].mean(axis=(0, 1))   # (valid_rows, Te)
fig, ax = plt.subplots(figsize=(8, 6))
fig.suptitle(
    f'Avg Self-Attention (all layers & heads) — Sim {chosen_sim}  ({N_subbox} sub-boxes {N_HALOS_SELECT} halos in sub-box)\n'
    f'x & y: decoder token position',
    fontsize=10,
)
...
plt.tight_layout()
save_path = os.path.join(OUT_DIR, f'self_avg_all_cosmo_{chosen_sim}.png')
plt.savefig(save_path, dpi=300, bbox_inches='tight')
plt.close(fig)
print(f'Saved: {save_path}')

np.save(os.path.join(OUT_NPY_DIR, f'self_avg_laryer_head_cosmo_{chosen_sim}.npy'), attn_all_avg)
np.save(os.path.join(OUT_NPY_DIR, f'self_avg_head_mean_per_layer_{chosen_sim}.npy'), attn_head_avg)

# Per-layer, per-head attention (no averaging over layers/heads)
attn_per_layer_head = cross_avg[:, :, :valid_rows, :valid_rows]   # (n_layers, n_heads, valid_rows, valid_rows)
np.save(os.path.join(OUT_NPY_DIR, f'self_attn_per_layer_head_cosmo_{chosen_sim}.npy'), attn_per_layer_head)
