...
class HaloDecoderModel(nn.Module):
    ...
    def forward(self, idx, density_all, params=None, maskd=None, targets=None):
        ...
        hnum = idx[:,6].unsqueeze(-1).expand(-1,idx.shape[-1]).float()
        weights = torch.where(hnum > 0, 1.0 / (halonum + 1e-6), torch.ones_like(hnum))
        ...
        if targets is not None:
            ...
            if self.config.loss_type == 'cross_entropy':
                ...
                '''    
                loss_per_token  = F.cross_entropy(logits.reshape(-1, logits.size(-1)),targets.reshape(-1),ignore_index=self.config.pad_token,reduction='none')
                mask = (targets.reshape(-1) != self.config.pad_token)
                loss = (loss_per_token[mask] * weights_flat[mask]).sum() / weights_flat[mask].sum()
