...
class HaloDecoderModel(nn.Module):
    ...
    def __init__(self, config_dict):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            wothere = nn.Embedding(config.vocab_size, config.n_embd),
    ...
    def forward(self, idx, density_all, params=None, maskd=None, targets=None):
        ...
        tok_emb = self.transformer.wte(idx[:,7:halo_end_max].long())
        tok_emb += (haloid_emb_full.unsqueeze(0).expand(b,-1,-1)+prop_emb_full.unsqueeze(0).expand(b,-1,-1)) * halo_mask.unsqueeze(2)
        cosmo_emb = self.transformer.wothere(idx[:,0:7].long())
        cosmo_emb[:,1:7,:] += self.transformer.wce(torch.arange(0,6, dtype=torch.long, device=device))  # (6, n_embd)
        tok_emb = torch.cat((cosmo_emb, tok_emb), dim=1)
    ...
    def generate(self, density_all, params=None, max_new_tokens=100, temperature=1.0, top_k=None, start_token=1, end_token=None, pad_token=None):
        ...
        tok_emb = self.transformer.wothere(idx.long())
        ...
        tok_emb = self.transformer.wothere(idx[:, -1:].long()) # token embeddings of shape (b, 1, n_embd)
