...
from cbam_res import *


class PatchEmbedding3D(nn.Module):
    def __init__(self, in_channels, embed_dim, patch_size, layers_types=['cnn']):
        super().__init__()
        if 'cnn' in layers_types[0]:
            self.proj = nn.Conv3d(in_channels, embed_dim, 
                                kernel_size=patch_size, stride=patch_size, bias=False, dtype=torch.bfloat16)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape # N: number of token; C: embedding dim
...
class Vision3DTransformer(nn.Module):
    def __init__(
        ...
        self.wce = nn.Embedding(cosmo_bins, embed_dim)
        self.wpe = nn.Embedding(5, embed_dim)
    ...
    def forward(self, x, cosmo_tokens):
        # x: (B, C, D1, D2, D3)
        ...
        # Add cosmological embeddings
        cosmo_emb = self.wce(cosmo_tokens.long())  # (B,5,embed_dim)
        cosmo_emb += self.wpe(torch.arange(0,5, dtype=torch.long, device=x.device)[None, :])

        x = torch.cat([cosmo_emb, x], dim=1)
