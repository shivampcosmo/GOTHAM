...
def train():
    ...
    cp_name = '/work/hdd/bdne/yzhang116/checkpoints_quijote/1831011/checkpoint_res_cross_entropy_epoch_0_step_42000_embed_384_batch_800_lrmax_0.0001_lrmin_1e-05_layer_8_head_12_layervit_4_headvit_8_patch_4_dropout_0.0.pt'
    ...
    if dist.get_rank() == 0:
        ...
        wandb.init(
            project='quijote',
            name='cos_%s_embed%d_lrmax%e_batch%d'%(loss_type,n_embd,lr_max,sub_batch_size*Ndevices),
            config={
                "epochs": max_iters,
        ...
        for batch_idx, batch_data in enumerate(dali_iterator):
            ...
            if sub_step % (nsub_batches*1) == 0:
                ...
                if dist.get_rank() == 0:
                    ...
                    check_point_name = os.path.join(job_dir, f'checkpoint_res_{loss_type}_epoch_{jepoch}_step_{sub_step}_embed_{n_embd}_batch_{sub_batch_size*Ndevices}_lrmax_{lr_max}_lrmin_{lr_min}_layer_{n_layer}_head_{n_head}_layervit_{n_layers_vit}_headvit_{n_heads_vit}_patch_{patch_size}_dropout_{dropout}.pt')
                ...
                if dist.get_rank() == 0:
                    ...
                    plot_name = os.path.join(plot_dir, f"performance_step_{sub_step}_{job_id}_res.png")
