...
class Block(nn.Module):
    ...
    def forward(self, x, xe=None, maskd=None, maske=None, return_kv=False, use_kv_cache=False, k_cache=None, v_cache=None):
        if use_kv_cache and k_cache is not None and v_cache is not None:
            ...
            y, self_attention = self.selfattn(self.ln_1(x), xe=None, maskd=maskd, maske=None)
            x = x + y
            y, cross_attetion = self.crossattn(self.ln_2(x), xe=xe, maskd=maskd, maske=None)
            x = x + y
            x = x + self.mlp(self.ln_3(x))

        if return_kv:
            return x, k_self, v_self            
        return x, self_attention, cross_attetion
...
class HaloDecoderModel(nn.Module):
    ...
    def forward(self, idx, density_all, params=None, maskd=None, targets=None):
        ...
        for block in self.transformer.h:
            x, self_attention, cross_attetion = block(x, xe=xe, maskd=maskd)    # x: galaxy tokens, xe: features of density field
            attn_self_all.append(self_attention)
            attn_cross_all.append(cross_attetion)
        attn_self_all = torch.stack(attn_self_all, dim=0)
        attn_cross_all = torch.stack(attn_cross_all, dim=0)
        ...
        return attn_self_all, attn_cross_all, logits
