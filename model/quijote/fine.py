...
def train():
    ...
    #cp_name = '/work/hdd/bdne/yzhang116/checkpoints_quijote/1830775/checkpoint_cos_cross_entropy_epoch_0_step_30000_embed_384_batch_800_lrmax_0.0001_lrmin_1e-05_layer_8_head_12_layervit_4_headvit_8_patch_4_dropout_0.0.pt'
    cp_name = f'/work/hdd/bdne/yzhang116/checkpoints_quijote/{cp_id}/checkpoint_cos_cross_entropy_epoch_0_step_{cp_step}_embed_384_batch_800_lrmax_1e-05_lrmin_1e-05_layer_8_head_12_layervit_4_headvit_8_patch_4_dropout_0.0.pt'
    ...
    def lr_lambda(step, total_steps=10000):
    ...
    if dist.get_rank() == 0:
        ...
        wandb.init(
            project='quijote',
            name='cp_%d_step_%d_lrmax%e_batch%d'%(cp_id,cp_step,lr_max,sub_batch_size*Ndevices),
            config={
                "epochs": max_iters,
    ...
    for jepoch in range(0, max_iters):
        for batch_idx, batch_data in enumerate(dali_iterator):
            ...
            for js in range(nsub_batches):
                ...
                if sub_step % 50 == 0 and sub_step > 0:
                    model.eval()
                    if dist.get_rank() == 0:
                        checkpoint = {
                        ...
                        check_point_name = os.path.join(job_dir, f'checkpoint_cos_{loss_type}_epoch_{jepoch}_step_{sub_step}_embed_{n_embd}_batch_{sub_batch_size*Ndevices}_lrmax_{lr_max}_lrmin_{lr_min}_layer_{n_layer}_head_{n_head}_layervit_{n_layers_vit}_headvit_{n_heads_vit}_patch_{patch_size}_dropout_{dropout}.pt')
