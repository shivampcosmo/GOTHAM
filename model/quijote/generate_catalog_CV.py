...
dmo_dir = '/work/hdd/bdne/yzhang116/quijote/test_3gpc/dmo/fastpm/'
...
if 1:
    ...
    param = np.array([0.3175, 0.049, 0.6711, 0.9624, 0.834])
    param = np.repeat(param[None,:], nsubox, axis=0)
    params_rep = torch.tensor(param, device=dev).bfloat16()
    dmo = np.load(dmo_dir+'dmo_fields_subvols_grid_8_CV_0_1gpc_corr.npy')
    ...
    end_time = time.time()
    print(f"Generation time : {(end_time-start_time)/60} mins", flush=True)
    data = np.int16(data)
    ...
    output_dir = '/work/hdd/bdne/yzhang116/quijote/test_3gpc/halo'
    os.makedirs(output_dir, exist_ok=True)
    out_filename =  os.path.join(output_dir, f'generated_halo_catalog_CV{simid}_fastpm_1gpc_corr.npy')
