...
def train():
    ...
    start_token_orig = meta_f['start_token']
    pad_token_orig = meta_f['pad_token']
    end_token_orig = meta_f['end_token']
    start_token = 41
    pad_token = 42
    end_token = 43
    ...
    if dist.get_rank() == 0:
        ...
        wandb.init(
            project='quijote',
            name='cos_vcab40_%s_embed%d_lrmax%e_batch%d'%(loss_type,n_embd,lr_max,sub_batch_size*Ndevices),
            config={
                "epochs": max_iters,
    ...
    for jepoch in range(0, max_iters):
        for batch_idx, batch_data in enumerate(dali_iterator):
            ...
            temp_halo = gal_tokens[:,7:-1].reshape(B,36,8)
            temp_prop = torch.round(temp_halo[:,:,3:]*40/131)
            temp_prop[temp_prop>40] = pad_token
            temp_pos = temp_halo[:,:,:3]
            temp_pos[temp_pos==pad_token_orig] = pad_token
            temp_pos[temp_pos==end_token_orig] = end_token
            temp_halo = torch.cat([temp_pos, temp_prop], dim=-1).reshape(B, -1)
            temp_cosmo = torch.round(gal_tokens[:,1:6]*40/131)
            temp_end = gal_tokens[:,-1:]
            temp_end[temp_end==end_token_orig] = end_token
            temp_end[temp_end==pad_token_orig] = pad_token
            gal_tokens = torch.cat([gal_tokens[:,0:1], temp_cosmo, gal_tokens[:,6:7], temp_halo, temp_end], dim=-1)
            gal_tokens[:,0] = start_token
            ...
            if sub_step % 5000 == 0 and batch_idx > 0:
                ...
                with torch.no_grad():
                    for batch_idx_val, batch_data_val in enumerate(dali_iterator_val):
                        ...
                        temp_halo = gal_tokens_val[:,7:-1].reshape(B,36,8)
                        temp_prop = torch.round(temp_halo[:,:,3:]*40/131)
                        temp_prop[temp_prop>40] = pad_token
                        temp_pos = temp_halo[:,:,:3]
                        temp_pos[temp_pos==pad_token_orig] = pad_token
                        temp_pos[temp_pos==end_token_orig] = end_token
                        temp_halo = torch.cat([temp_pos, temp_prop], dim=-1).reshape(B, -1)
                        temp_cosmo = torch.round(gal_tokens_val[:,1:6]*40/131)
                        temp_end = gal_tokens_val[:,-1:]
                        temp_end[temp_end==end_token_orig] = end_token
                        temp_end[temp_end==pad_token_orig] = pad_token
                        gal_tokens_val = torch.cat([gal_tokens_val[:,0:1], temp_cosmo, gal_tokens_val[:,6:7], temp_halo, temp_end], dim=-1)
                        gal_tokens_val[:,0] = start_token
                ...
                if loss_mean_here < best_loss+0.1 and dist.get_rank() == 0:
                        checkpoint = {
