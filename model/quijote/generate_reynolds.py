import torch
import numpy as np
from model_enc_dec_reflect import *
import numpy as np
import glob
import os
import sys
...
for i in range(len(model.transformer.h)):
    model.transformer.h[i] = torch.compile(model.transformer.h[i], 
        mode='default')
...
if 1:
    ...
    with torch.no_grad():
        with ctx:
            for i in range(num_chunks):
                ...
                torch.manual_seed(simid * num_chunks + i)
                torch.cuda.manual_seed_all(simid * num_chunks + i)
                out_chunk = model.generate_with_reflection_reynolds(
                    dmo_chunk,
                    params=params_chunk,
                    max_new_tokens=289,
    ...
    end_time = time.time()
    print(f"Generation time for Om {all_param[simid,0]}: {(end_time-start_time)/60} mins", flush=True)
    data = np.int16(data)
    np.save(f'/work/hdd/bdne/yzhang116/generate_catalog_quijote/reynolds_reflect/generated_halo_sentence_{simid}_4config.npy', data)
    #exit(0)
    ...
    output_dir = '/work/hdd/bdne/yzhang116/generate_catalog_quijote/reynolds_reflect'
    os.makedirs(output_dir, exist_ok=True)
    out_filename =  os.path.join(output_dir, f'generated_halo_catalog_{simid}_4config.npy')
    np.save(out_filename, catalog)
    print(f'Saved generated halos for sim {simid} to {out_filename}', flush=True)
    ...
    with torch.no_grad():
        with ctx:
            for i in range(num_chunks):
                ...
                torch.manual_seed(simid * num_chunks + i)
                torch.cuda.manual_seed_all(simid * num_chunks + i)
                out_chunk = model.generate_with_reflection_reynolds_8config(
                    dmo_chunk,
                    params=params_chunk,
                    max_new_tokens=289,
                    start_token=start_token,
    ...
    print(f"Generation time for Om {all_param[simid,0]}: {(end_time-start_time)/60} mins", flush=True)
    data = np.int16(data)
    np.save(f'/work/hdd/bdne/yzhang116/generate_catalog_quijote/reynolds_reflect/generated_halo_sentence_{simid}_8config.npy', data)
    #exit(0)
    data = np.delete(data, [1,2,3,4,5,6], axis=1)
    data = data.reshape((grid,grid,grid,-1))
    np.random.seed(simid)
    ...
    output_dir = '/work/hdd/bdne/yzhang116/generate_catalog_quijote/reynolds_reflect'
    os.makedirs(output_dir, exist_ok=True)
    out_filename =  os.path.join(output_dir, f'generated_halo_catalog_{simid}_8config.npy')
    np.save(out_filename, catalog)
    print(f'Saved generated halos for sim {simid} to {out_filename}', flush=True)