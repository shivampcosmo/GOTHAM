...
import torch.optim as optim
import pickle as pk
from model_enc_dec_hnum import *
...
def train():
    ...
    if dist.get_rank() == 0:
        ...
        wandb.init(
            project='quijote',
            name='Om_%s_embed%d_lrmax%e_batch%d'%(loss_type,n_embd,lr_max,sub_batch_size*Ndevices),
            config={
                "epochs": max_iters,
    ...
    for jepoch in range(0, max_iters):
        for batch_idx, batch_data in enumerate(dali_iterator):
            ...
            DM[:, dens_lin_idx] = (DM[:,dens_lin_idx] * Oms[:,None,None,None,None] / 0.3).to(DM.dtype)  # scale density
            ...
            if sub_step % (nsub_batches*50) == 0:
                ...
                with torch.no_grad():
                    for batch_idx_val, batch_data_val in enumerate(dali_iterator_val):
                        ...
                        Y_val[:, :5] = pad_token
                        Oms = gal_tokens_val[:, 1] / 131 * 0.4 + 0.1
                        ...
                        dens_lin_idx = torch.tensor([0, 2, 4, 6, 10, 12, 14])
                        DM_val[:, dens_lin_idx] = (DM_val[:,dens_lin_idx] * Oms[:,None,None,None,None] / 0.3).to(DM_val.dtype)  # scale density
                ...
                if dist.get_rank() == 0:
                    ...
                    plot_name = f"/u/yzhang116/NN/plots/performance_step_{sub_step}_{job_id}.png"
