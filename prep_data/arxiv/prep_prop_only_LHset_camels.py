...
def process_LH_sim(isim_fid):
    ...
    # snapdir = snap_dir_base + '/LH/' + f'LH_{isim_fid}'  #folder hosting the catalogue
    ...
    group_catalog = f'{snap_dir_base}/LH/LH_{LHid}/groups_090.hdf5'
    photo_catalog = f'{snap_dir_base}/Photometry/IllustrisTNG/L25n256/LH/IllustrisTNG_LH_{LHid}_photometry.hdf5'
    # open the catalogue
    ...
    if os.path.exists(savefname):
        ...
        # grid = 8
        # nMax_h = 42
        dfhalo_ngp_wxyzM = np.float32(np.zeros((grid, grid, grid, nMax_h, prop_truth_all.shape[0] + pos_h_truth.shape[0])))
