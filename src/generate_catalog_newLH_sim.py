...
simid = int(sys.argv[1]) - 2000
output_dir = '/mnt/ceph/users/spandey/discodj_runs/quijote_generated_halo_cats'
os.makedirs(output_dir, exist_ok=True)
out_filename =  os.path.join(output_dir, f'generated_halo_catalog_{simid+2000}.npy')

# check if the file doesn't exist the execute, else not
if os.path.exists(out_filename):
    print(f'File {out_filename} already exists, skipping generation for sim {simid+2000}')
    sys.exit(0)
else:
    ...
    all_param = np.loadtxt('/mnt/ceph/users/spandey/quijote_v2_gotham/GOTHAM/checkpoints/new_LH_sims_params.txt')
    ...
    files = [dmo_dir + f'{simid+2000}/dmo_fields_subvols_grid_8_LH_{simid+2000}.npy' for simid in simids]
    ...
    dmo = np.load(dmo_dir+f'{simid+2000}/dmo_fields_subvols_grid_8_LH_{simid+2000}.npy')
