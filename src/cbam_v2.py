...
#         # Main path
#         self.conv1 = nn.Conv3d(in_channels, out_channels//2, kernel_size=kernel_size, stride=stride, padding=0, bias=False, dtype=torch.bfloat16)
...
#             # )
#         self.skip = nn.Sequential(
#             nn.Conv3d(in_channels, out_channels, kernel_size=1, stride=2, bias=False, dtype=torch.bfloat16),
#             # nn.BatchNorm3d(out_channels)
#         )
...
class ResBlock3D_with_CBAM(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1):
        ...
        # Main path
        self.conv1 = nn.Conv3d(in_channels, out_channels//8, kernel_size=kernel_size, stride=stride, padding=0, bias=False, dtype=torch.bfloat16)
        self.bn1 = nn.RMSNorm(out_channels//8)
        self.relu1 = nn.LeakyReLU(inplace=True)
        
        self.conv2 = nn.Conv3d(out_channels//8, out_channels//4, kernel_size=kernel_size, padding=0, bias=False)
        self.bn2 = nn.RMSNorm(out_channels//4)
        self.relu2 = nn.LeakyReLU(inplace=True)

        self.conv3 = nn.Conv3d(out_channels//4, out_channels//2, kernel_size=kernel_size, padding=0, bias=False)
        self.bn3 = nn.RMSNorm(out_channels//2)
        self.relu3 = nn.LeakyReLU(inplace=True)

        self.conv4 = nn.Conv3d(out_channels//2, out_channels, kernel_size=kernel_size, padding=0, bias=False)
