...
class PairedDataset(Dataset):
    def __init__(self, x_dir, y_dir, num=None):
        rank = int(os.environ.get("RANK", 0))
        world_size = int(os.environ.get("WORLD_SIZE", 1))

        x_files = sorted(glob.glob(os.path.join(x_dir, "*.npy")), key=get_sim_number)[:num]
        y_files = sorted(glob.glob(os.path.join(y_dir, "*.npy")), key=get_sim_number)[:num]
...
def train():
    ...
    meta_f = pk.load(open('/work/hdd/bdne/yzhang116/halo_sentences_36/sentence_params_36.pkl','rb'))
    ...
    dmo_dir = '/work/hdd/bdne/yzhang116/dmo_fields_4096_snap3/train/'
    halo_sentence_dir = '/work/hdd/bdne/yzhang116/halo_sentences_36/train/'
    ...
    vali_dataset = PairedDataset(halo_sentence_dir_val, dmo_dir_val,valinum)
    batch_size_val = 125
    ...
    train_dataset = PairedDataset(halo_sentence_dir, dmo_dir,cosnum)
    batch_size = 125
    ...
    if dist.get_rank() == 0:
        ...
        wandb.init(
            project='camel-sam',
            name='36nvocab_hnumv3_%s_embed%d_lrmax%e_batch%d'%(loss_type,n_embd,lr_max,batch_size*Ndevices),
            config={
                "epochs": max_iters,
    ...
    for jepoch in range(0, max_iters):
        for batch_idx, batch_data in enumerate(train_dataloader):
            ...
            n_halo_actual = (end_token_index - 3) // 8  # shape: (b,)
            n_halo_actual = torch.clamp(n_halo_actual, min=0, max=64)
            X = torch.cat([gal_tokens[:,:3],n_halo_actual.unsqueeze(1),gal_tokens[:,3:-1]],dim=1).long()
            ...
            if batch_idx % 1000 == 0: # and batch_idx > 0:
                ...
                with torch.no_grad():
                    for val_batch_idx, val_batch_data in enumerate(vali_dataloader):
                        ...
                        n_halo_actual_val = (end_token_index_val - 3) // 8  # shape: (b,)
                        n_halo_actual_val = torch.clamp(n_halo_actual_val, min=0, max=64)
                        X_val = torch.cat([gal_tokens_val[:,:3],n_halo_actual_val.unsqueeze(1),gal_tokens_val[:,3:-1]],dim=1).long()
                        Y_val = torch.cat([torch.zeros(B,1,device=device), gal_tokens_val[:,1:]],dim=1).long()
                        Y_val[:, :2] = pad_token
                        Y_val[:, 2] = n_halo_actual_val
                ...
                if loss_mean_here < best_loss:
                    ...
                    if dist.get_rank() == 0:
                        ...
                        check_point_name = f'/work/hdd/bdne/yzhang116/checkpoints_36/checkpoint_36_hnumv3_{loss_type}_epoch_{jepoch}_batch_{batch_idx}_embed_{n_embd}_batch_{batch_size*Ndevices}_lrmax_{lr_max}_lrmin_{lr_min}_layer_{n_layer}_head_{n_head}_layervit_{n_layers_vit}_headvit_{n_heads_vit}_patch_{patch_size}_dropout_{dropout}_shuffle_{shuffle}.pt'
