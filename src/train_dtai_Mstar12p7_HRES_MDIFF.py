...
def train():
    ...
    savefname = f'{sdir}/SPLIT_HALO_DATA_{Ndevices}_gpus_isim_all_nrandsubsel_{int(nrand_sel_box/subsamp_ds)}_grid_{nvocab//DS_RES_POS_FAC}_nvocab{nvocab}_spacetoken_{add_space_token}_xyzMvc_{Mstar_cut}.h5'
    ...
    dist.barrier()
    with h5.File(savefname, 'r') as f:
        x_train_gpu = torch.tensor(f[f'x_train_dev_{rank}'][:][ind_sel_train]).to(dtype_int).to(device_id, non_blocking=True)
        y_train_gpu = torch.tensor(f[f'y_train_dev_{rank}'][:][ind_sel_train]).to(dtype_int).to(device_id, non_blocking=True)
        ...
        x_val_gpu = torch.tensor(f[f'x_val_dev_{rank}'][:][ind_sel_val]).to(dtype_int).to(device_id, non_blocking=True)
        y_val_gpu = torch.tensor(f[f'y_val_dev_{rank}'][:][ind_sel_val]).to(dtype_int).to(device_id, non_blocking=True)
    ...
    # cp_name = '/projects/bdne/spandey3/halo_gotham/GOTHAM/model_checkpoints/quijote_halos/FULL_wposembed_vit/TEST_xMv1Dc_LRES_MDIFF_cnn_vit_cbam_ps_1_nrandsubsel_8192_cross_entropy_iter_84_seq_grid_8_nvocab_64_nembed_128_nhead_4_nrandsubsel_8192_subselDMOfields_all_Mstarcut_12.7_maxiter_600_lr_0.0002.pt'    
        ...
        if iter_num % eval_interval == 0 and (rank == 0):
                ...
                if losses['val'] < best_val_loss:
                    ...
                    if iter_num > 0:
                        ...
                        torch.save(checkpoint, f'/projects/bdne/spandey3/halo_gotham/GOTHAM/model_checkpoints/quijote_halos/FULL_HRES_wposembed_vit/TEST_xyzMvc_LRES_MDIFF_cnn_{cnn_type}_ps_{patch_size}_nrandsubsel_{int(nrand_sel_box/subsamp_ds)}_{loss_type}_iter_{iter_num}_{ds_type_here}_grid_{grid_sbox}_nvocab_{nvocab}_nembed_{n_embd}_nhead_{n_head}_nrandsubsel_{int(nrand_sel_box/(subsamp_ds * ds_fac_here))}_subselDMOfields_{subsel_type}_Mstarcut_{Mstar_cut}_maxiter_{max_iters}_lr_{learning_rate}.pt')                                 
