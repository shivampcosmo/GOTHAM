...
class ERA5ES(object):
    ...
    def _get_files_stats(self, enable_logging):
        ...
        if enable_logging:
            ...
            if self.num_shards > 1:
                logging.info(
                    "Using shards of size {} per rank".format(self.n_samples_shard)
                )
        ...
        self.inp_buffs = [
            cpx.zeros_pinned(
                (self.n_in_channels, self.img_shape_x, self.img_shape_y),
                dtype=np.float32,
            ),
            cpx.zeros_pinned(
                (self.n_in_channels, self.img_shape_x, self.img_shape_y),
        ...
        self.tar_buffs = [
            cpx.zeros_pinned(
                (self.n_out_channels, self.img_shape_x, self.img_shape_y),
                dtype=np.float32,
            ),
            cpx.zeros_pinned(
                (self.n_out_channels, self.img_shape_x, self.img_shape_y),
    ...
    def __call__(self, sample_info):
        ...
        local_idx = int(
            sample_idx % self.n_samples_per_year
