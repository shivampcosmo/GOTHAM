...
def process_LH_sim(sim_id,grid_sbox = 8):
    try:
        grid = 16
        ...
        savefname_dmo_fields = '/work/hdd/bdne/yzhang116/dmo_fields_4096_256/dmo_fields_LH_%d.npy'%(sim_id)  
        ...
        for js, snapnum in enumerate(snaps):
            input_pos = '/work/hdd/bdne/yzhang116/ICsSAM/cLH%d/PM/pos_cLH%d_z%s_256.npy'%(sim_id,sim_id,snapnum)
            input_vel = '/work/hdd/bdne/yzhang116/ICsSAM/cLH%d/PM/vel_cLH%d_z%s_256.npy'%(sim_id,sim_id,snapnum)
            import numpy as np
            np.random.seed(0)
            pos_m_truth = np.load(input_pos)
            vel_m_truth = np.load(input_vel)
...
if __name__ == '__main__':
    for sim_id in numbers:
        process_LH_sim(sim_id, grid_sbox=16)
